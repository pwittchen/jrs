...
| `ROADMAP.md` | What is **left** to do. Its opening paragraph lists what has landed. | Grep the code for each remaining item. An item that has been implemented moves out of its section into the "has landed" sentence; don't mark it done in place. Adding new items is the user's call — suggest, don't add. |
...
- **Lists that should be complete**: the crate list, CI jobs and OSes,
  ...
  README feature list, the ROADMAP "landed" sentence.
