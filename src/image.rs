...
pub fn jlink(
    ...
    std::fs::write(&sh, sh_launcher(app.jvm_args, jar_name)).path(&sh)?;
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
        std::fs::set_permissions(&sh, std::fs::Permissions::from_mode(0o755)).path(&sh)?;
    }
    std::fs::write(&bat, bat_launcher(app.jvm_args, jar_name)).path(&bat)?;
...
/// arguments, as the conventional escape hatch.
fn sh_launcher(jvm_args: &[String], jar_name: &str) -> String {
...
/// with them.
fn bat_launcher(jvm_args: &[String], jar_name: &str) -> String {
...
mod tests {
    ...
    fn launchers_are_deterministic() {
        ...
        assert_eq!(sh_launcher(&args, "a.jar"), sh_launcher(&args, "a.jar"));
        assert_eq!(bat_launcher(&args, "a.jar"), bat_launcher(&args, "a.jar"));
    }

    #[test]
    fn the_bat_launcher_runs_the_images_own_java() {
        let text = bat_launcher(&strings(&["-Xmx512m", "-Dgreeting=hello world"]), "app.jar");
        assert_eq!(
            text,
            "@echo off\r\n\
             rem Generated by jrs.\r\n\
